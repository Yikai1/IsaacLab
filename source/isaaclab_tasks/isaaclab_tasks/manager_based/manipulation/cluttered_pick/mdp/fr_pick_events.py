...
def set_default_object_pose(
    ...
    for cur_env in env_ids.tolist():
        ...
        for i in range(len(asset_cfgs)):
            ...
            init_pos = torch.tensor(asset.cfg.init_state.pos, device=env.device).unsqueeze(0).repeat(env.num_envs, 1)
            init_rot = torch.tensor(asset.cfg.init_state.rot, device=env.device).unsqueeze(0).repeat(env.num_envs, 1)
            asset.write_root_pose_to_sim(
                    torch.cat([init_pos, init_rot], dim=-1), env_ids=torch.tensor([cur_env], device=env.device)
                )
